...
@pytest.mark.asyncio
async def test_executed_tool_loop_is_also_stopped_after_identical_results() -> None:
    ...
    class ConstantTool:
        ...
        async def execute(self, arguments: BaseModel) -> ToolOutput:
            return ToolOutput("still failing")

    provider = FakeProvider(
        [
            AssistantExchange(
                (ToolUseBlock(f"call-{index}", "check", {}),),
                "tool_use",
            )
            for index in range(3)
        ]
    ...
    application = AgentApplication(
        provider,
        ToolDispatcher(ToolCatalog({"check": ConstantTool()})),
        sessions,
    )

    events = [event async for event in application.run("Keep checking")]
